...
class VentJar {
 ...
 async loadMessages() {
  ...
  container.innerHTML = '<div class="loading-indicator">Loading your thoughts...</div>';
  ...
  try {
   if (this.isGuest) {
    ...
    if (data.success) {
     ...
     container.innerHTML = '<div class="empty-jar-message">Your jar is empty. Write something to fill it!</div>';
    }
   }
  } catch (error) {
   console.error('Error loading messages:', error);
   container.innerHTML = '<div class="empty-jar-message">⚠️ Error loading messages. Please refresh the page.</div>';
  }
 }
 ...
 async addMessage() {
  ...
  try {
   ...
   putButton.innerHTML = '<i class="ti ti-circle-dashed-plus"></i><span>Put</span>';
  }
 }
 ...
 async burnAllMessages() {
  ...
  const confirmed = confirm('⚠️ Are you sure? This will burn ALL your messages permanently.');
  if (!confirmed) return;
  ...
  if (this.isGuest) {
   ...
   const deletePromises = this.messages.map(msg => this.deleteMessage(msg.id));
 ...
 renderPapers() {
  ...
  // Calculate random positions within the jar
 ...
 createPaperElement(message, index) {
  ...
  // Create random position within jar
  const randomX = 10 + Math.random() * 60; // 10% to 70% of container width
  const randomY = 20 + Math.random() * 60; // 20% to 80% of container height
...
// Initialize the vent feature when DOM is loaded
