class HopeFeature {
 ...
 renderEmptyStories() {
  const track = document.getElementById("storiesTrack");
  track.innerHTML =
   '<div class="slide">No heartbreak stories available at the moment.</div>';
 }
  ...
  try {
   ...
   if (healingData.success && healingData.books) {
    ...
    this.renderEmptyBooks(
     "healingBooksAccordion",
     "No healing books available yet.",
    );
   }
 ...
 renderBooks(books, containerId) {
  ...
  if (!books || books.length === 0) {
   container.innerHTML =
    '<div class="accordion-item">No books available in this category.</div>';
   return;
  }
